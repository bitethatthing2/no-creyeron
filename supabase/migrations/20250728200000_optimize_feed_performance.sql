...
CREATE INDEX IF NOT EXISTS idx_content_posts_feed_performance 
ON content_posts (is_active, created_at DESC) 
WHERE is_active = true;

-- User video queries optimization
CREATE INDEX IF NOT EXISTS idx_content_posts_user_timeline 
ON content_posts (user_id, is_active, created_at DESC) 
WHERE is_active = true;

-- Video stats queries optimization
CREATE INDEX IF NOT EXISTS idx_content_posts_stats 
ON content_posts (id, like_count, content_comments_count, view_count);
...
-- Video content_comments for feed display
CREATE INDEX IF NOT EXISTS idx_content_comments_video_active 
ON content_comments(video_id, is_deleted, created_at DESC) 
WHERE is_deleted = false;

-- User content_comments lookup
CREATE INDEX IF NOT EXISTS idx_content_comments_user_active 
ON content_comments(user_id, is_deleted, created_at DESC) 
WHERE is_deleted = false;
...
CREATE INDEX IF NOT EXISTS idx_content_posts_realtime 
ON content_posts (created_at DESC, updated_at DESC) 
WHERE is_active = true;
...
SELECT 
  ...
  COALESCE(SUM(v.content_comments_count), 0) as total_content_comments,
  COALESCE(SUM(v.view_count), 0) as total_views,
  MAX(v.created_at) as last_video_at
FROM users u
LEFT JOIN content_posts v ON u.id = v.user_id AND v.is_active = true
...
DROP TRIGGER IF EXISTS update_video_comment_stats ON content_comments;
CREATE TRIGGER update_video_comment_stats
  AFTER INSERT OR UPDATE OR DELETE ON content_comments
  FOR EACH ROW
  EXECUTE FUNCTION trigger_update_video_stats();
...
RETURNS TABLE(
  total_content_posts INTEGER,
  active_content_posts INTEGER,
  avg_likes_per_video NUMERIC,
  avg_content_comments_per_video NUMERIC,
...
BEGIN
  RETURN QUERY
  SELECT 
    COUNT(*)::INTEGER as total_content_posts,
    COUNT(*) FILTER (WHERE is_active = true)::INTEGER as active_content_posts,
    AVG(like_count) as avg_likes_per_video,
    AVG(content_comments_count) as avg_content_comments_per_video,
...
-- content_comments for documentation
