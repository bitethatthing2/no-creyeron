...
-- ===================================
-- 4. Fix content_posts policies
-- ===================================

-- Drop duplicate policies
DROP POLICY IF EXISTS "anyone_can_view_active_content_posts" ON content_posts;
DROP POLICY IF EXISTS "wolfpack_members_view_content_posts" ON content_posts;

-- Create single consolidated policy
CREATE POLICY "content_posts_select_consolidated" ON content_posts
FOR SELECT TO authenticated
USING (
    is_active = true AND 
    (
...
CREATE INDEX IF NOT EXISTS idx_content_posts_active_published ON content_posts(is_active, is_published);
...
COMMENT ON POLICY "content_posts_select_consolidated" ON content_posts IS 'Consolidated policy - active published content_posts visible to all, owners and admins see their own';