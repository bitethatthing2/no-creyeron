...
function processMediaUrls(item: any): MenuItemRecord {
  const STORAGE_BASE = 'https://tvnpgbjypnezoasbhbwx.supabase.co/storage/v1/object/public';
  
  const processUrl = (url: string | null): string | undefined => {
  ...
  return {
    ...
    thumbnail_url: processUrl(item.thumbnail_url),
  };
}
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Processing request:', { originalPath: url.pathname, processedPath: path, fullUrl: req.url });
    ...
    switch (path) {
      ...
      case '/list': {
        ...
        const filters: MenuFilters = {
          ...
          is_available: searchParams.get('is_available') ? searchParams.get('is_available') === 'true' : undefined,
        ...
        const sortBy = filters.sort_by === 'sort_order' ? 'display_order' : filters.sort_by || 'display_order';
        query = query
      ...
      case '/search': {
        ...
        if (additionalFilters.category) searchQuery = searchQuery.eq('category_id', additionalFilters.category);
        if (additionalFilters.min_price) searchQuery = searchQuery.gte('price', additionalFilters.min_price);
        if (additionalFilters.max_price) searchQuery = searchQuery.lte('price', additionalFilters.max_price);
        if (additionalFilters.featured) searchQuery = searchQuery.eq('is_featured', additionalFilters.featured);
        if (additionalFilters.spice_level) searchQuery = searchQuery.lte('spice_level', additionalFilters.spice_level);
        if (additionalFilters.prep_time_max) searchQuery = searchQuery.lte('prep_time_minutes', additionalFilters.prep_time_max);
      ...
      case '/types': {
        ...
        return jsonResponse({
          ...
          data: [],
          total: 0,
          message: "Types endpoint not available - schema uses category_id instead"
        });
      }
      ...
      case '/categories': {
        ...
        if (error) {
          ...
          return jsonResponse({
            ...
            data: [],
            total: 0,
            message: "Categories endpoint not available - menu_categories table not found"
          });
        }
      ...
      case '/grouped': {
        ...
        if (!groupBy || !['type', 'category'].includes(groupBy)) {
          return errorResponse('Query parameter "group_by" must be "type" or "category"', 400);
        }
        ...
        return jsonResponse({
          ...
          total_groups: 0,
          total_items: 0,
          message: `Grouped by ${groupBy} not available - current schema uses category_id references`
        });
      }
