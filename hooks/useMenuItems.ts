...
export const useMenuItems = (filters: MenuFilters = {}) => {
  ...
  const { is_active, is_available, limit, offset, category, featured, search, sort_by, sort_order } = filters;
  ...
  }, [is_active, is_available, limit, offset, category, featured, search, sort_by, sort_order]); // Use specific filter properties

  // Use useCallback for stable function reference
  const searchItems = useCallback(async (query: string, additionalFilters: Omit<MenuFilters, 'search'> = {}) => {
    setLoading(true);
    setError(null);
    
    try {
      const data = await menuApiService.searchMenuItems(query, additionalFilters);
      setItems(data.data);
      setTotalCount(data.total_count);
      setHasMore(data.pagination?.has_more || false);
    } catch (err) {
