    ...
    const checkDJPermissions = async () => {
      ...
      try {
        ...
        if (isVipUser || tempAllowAnyUser) {
          if (process.env.NODE_ENV === "development") {
            console.log("🎯 Setting DJ permissions for user:", {
              isVipUser,
              tempAllowAnyUser,
              userEmail: user?.email,
            });
        ...
        console.warn(
          "Could not check DJ permissions (table may not exist yet):",
          error,
        );
        ...
        if (isVipUser || tempAllowAnyUser) {
          console.log("🎯 Setting DJ permissions (catch block) for user:", {
            isVipUser,
            tempAllowAnyUser,
            userEmail: user?.email,
          });
