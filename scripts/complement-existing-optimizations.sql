...
-- Partial index for active wolfpack content_posts by location
CREATE INDEX IF NOT EXISTS idx_content_posts_active_location_date 
ON content_posts(location_tag, created_at DESC, user_id) 
WHERE is_active = true;

-- Index for comment counts (to support your automatic count updates)
CREATE INDEX IF NOT EXISTS idx_content_comments_video_active_date 
ON content_comments(video_id, created_at DESC) 
...
BEGIN
  ...
  FROM pg_indexes 
  WHERE tablename IN ('content_posts', 'users', 'content_comments', 'wolfpack_post_likes')
...
SELECT 
  ...
  (
    ...
    WHERE relname IN ('content_posts', 'users', 'content_comments', 'wolfpack_post_likes')
      AND idx_scan > 0
  ) as current_value,
