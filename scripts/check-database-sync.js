...
async function checkCriticalColumns() {
  ...
  // Check content_comments has correct column names (based on schema doc)
  try {
    const { data, error } = await supabase
    ...
    if (error) {
      console.error('❌ content_comments column check failed:', error.message)
      checks.push(false)
    } else {
      console.log('✅ content_comments columns are correct (video_id, parent_comment_id, is_deleted, etc.)')
      checks.push(true)
    }
  } catch (error) {
    console.error('❌ content_comments column check failed:', error.message)
