...
export function useFeedwolfpack_posts(limit = 20): Usewolfpack_postsReturn {
  const [wolfpack_posts, setwolfpack_posts] = React.useState<WolfpackVideo[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState<Error | null>(null);
  const [hasMore, setHasMore] = React.useState(true);
  const [offset, setOffset] = React.useState(0);

  const loadwolfpack_posts = React.useCallback(async (reset = false) => {
    try {
      setLoading(true);
      setError(null);
...
export function usePost(postId: string): UseSinglePostReturn {
  const [post, setPost] = React.useState<WolfpackVideo | null>(null);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState<Error | null>(null);
  ...
  const updatePostData = React.useCallback(async (updates: {
    title?: string;
    description?: string;
    thumbnail_url?: string;
  }) => {
  ...
  const deletePostData = React.useCallback(async () => {
    if (!postId) throw new Error("No post ID");
