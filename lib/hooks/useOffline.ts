...
export function useOffline(options: UseOfflineOptions = {}): UseOfflineReturn {
  ...
  const [syncItems, setSyncItems] = React.useState<number>(0);
  ...
  const createOrder = React.useCallback(async (orderData: OrderData): Promise<string> => {
  ...
  const updateProfile = React.useCallback(async (profileData: ProfileData): Promise<string> => {
    return updateOfflineProfile(profileData);
  }, []);
  
  // Submit feedback that works offline
  const submitFeedback = React.useCallback(async (feedbackData: FeedbackData): Promise<string> => {
    return submitOfflineFeedback(feedbackData);
  }, []);
  
  // Get pending sync items
  const getPendingItems = React.useCallback(async (): Promise<SyncItem[]> => {
    return getPendingSyncItems();
  }, []);
  
  // Register for periodic updates
  const registerPeriodicUpdate = React.useCallback(async (): Promise<boolean> => {
