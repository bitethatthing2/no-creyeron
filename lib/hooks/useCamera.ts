  ...
  const startCamera = useCallback(async (requestedFacingMode?: FacingMode) => {
    ...
    try {
      ...
      const assignVideoStream = (retries = 3) => {
        if (videoRef.current) {
          console.log(
            "[Camera] Setting video srcObject, current srcObject:",
          ...
          console.warn(
            "[Camera] videoRef.current is null! Retrying in 100ms...",
          );
          setTimeout(() => assignVideoStream(retries - 1), 100);
        } else {
          console.warn(
            "[Camera] videoRef.current is null! Max retries exceeded.",
          );
        }
      };
      ...
      if (error instanceof Error) {
        ...
        if (error.name === "NotAllowedError") {
          ...
          console.log(
            "[Camera] Start was aborted - likely from rapid component updates",
          );
          return;
        } else {
