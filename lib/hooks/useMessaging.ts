...
// Precise message type matching wolfpack_messages table schema
...
// Precise conversation type matching wolfpack_conversations table schema
...
// Precise participant type matching wolfpack_conversation_participants table schema
  ...
  const loadConversations = React.useCallback(async () => {
    ...
    try {
      ...
      const { data: convData, error: convError } = await supabase
        ...
        .select(`
          *,
          participants:wolfpack_conversation_participants(
            *,
            user:users(
              id,
              email,
        ...
        let query = supabase
          ...
          .select(`
            *,
            ...
            reply_to:wolfpack_messages!reply_to_id(
              id,
              content,
              sender_id,
              created_at
    ...
    async (otherUserId: string) => {
      ...
      try {
        ...
        const { data: existing } = await supabase
          ...
          .select(`
            conversation_id,
            conversation:wolfpack_conversations(
              id,
              conversation_type
            )
          `)
