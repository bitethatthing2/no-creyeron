...
> => {
  ...
  if (navigator.serviceWorker.controller) {
    console.log(
      "Service worker is already registered and controlling the page",
    );
    try {
      ...
      console.error(
        "Error getting existing service worker registration:",
        error,
      );
    }
  }
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      if (hasActiveServiceWorker) {
        ...
        console.warn(
          "No token received with existing service worker, will try clean registration",
        );
      } else {
      ...
      console.error(
        `Error getting FCM token (attempt ${retryCount + 1}/${
          maxRetries + 1
        }):`,
        error,
      );
      ...
      try {
        ...
        if ("Notification" in window && Notification.permission === "granted") {
          ...
          if (document.visibilityState !== "visible") {
            ...
            const title = dataObject.title || payload.notification?.title ||
              "New Message";
