...
export class EdgeFunctionTester {
  ...
  async testCleanupScheduler(): Promise<EdgeFunctionTestSuite> {
    ...
    tests.push(await this.runTest('Cleanup without auth', async () => {
      const response = await fetch(`${BASE_URL}/functions/v1/cleanup-scheduler`, {
        method: 'POST',
    ...
    tests.push(await this.runTest('Cleanup with auth', async () => {
  ...
  async testFeedProcessor(): Promise<EdgeFunctionTestSuite> {
    ...
    // Test 1: Get Feed
    tests.push(await this.runTest('Get feed', async () => {
      const response = await fetch(`${BASE_URL}/functions/v1/FEED_PROCESSOR/get-feed`, {
        method: 'POST',
        headers,
    ...
    tests.push(await this.runTest('Like post', async () => {
      // First get a post to like
      const feedResponse = await fetch(`${BASE_URL}/functions/v1/FEED_PROCESSOR/get-feed`, {
        method: 'POST',
        headers,
      ...
      const response = await fetch(`${BASE_URL}/functions/v1/FEED_PROCESSOR/like`, {
        method: 'POST',
        headers,
    ...
    // Test 3: Add Comment
    tests.push(await this.runTest('Add comment', async () => {
      ...
      const response = await fetch(`${BASE_URL}/functions/v1/FEED_PROCESSOR/comment`, {
        method: 'POST',
        headers,
        body: JSON.stringify({ 
          post_id: postId, 
  ...
  async testContentUploader(): Promise<EdgeFunctionTestSuite> {
    ...
    tests.push(await this.runTest('Upload small file', async () => {
      ...
      const response = await fetch(`${BASE_URL}/functions/v1/CONTENT_UPLOADER/upload`, {
        method: 'POST',
    ...
    tests.push(await this.runTest('Upload oversized file', async () => {
      ...
      if (response.status !== 400 && response.status !== 413) {
        const data = await response.json();
        throw new Error(`Expected 400 or 413, got ${response.status}: ${data.error}`);
      }
      
      return { status: response.status, data: 'Correctly rejected oversized file' };
    }));
  ...
  async testMessageHandler(): Promise<EdgeFunctionTestSuite> {
    ...
    tests.push(await this.runTest('Get conversations', async () => {
      const response = await fetch(`${BASE_URL}/functions/v1/MESSAGE_HANDLER/get-conversations`, {
        method: 'POST',
        headers,
    ...
    tests.push(await this.runTest('Create DM', async () => {
      const { data: { user } } = await this.supabase.auth.getUser();
      
      const response = await fetch(`${BASE_URL}/functions/v1/MESSAGE_HANDLER/create-dm`, {
        method: 'POST',
        headers,
  ...
  async testPushNotifications(): Promise<EdgeFunctionTestSuite> {
    ...
    tests.push(await this.runTest('Store FCM token', async () => {
      const response = await fetch(`${BASE_URL}/functions/v1/PUSH_NOTIFICATIONS/store-token`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
    ...
    tests.push(await this.runTest('Get notifications', async () => {
      const response = await fetch(`${BASE_URL}/functions/v1/PUSH_NOTIFICATIONS/get-notifications`, {
        method: 'POST',
        headers,
  ...
  async testMenuItems(): Promise<EdgeFunctionTestSuite> {
    ...
    tests.push(await this.runTest('Get food items', async () => {
      ...
      const response = await fetch(`${BASE_URL}/functions/v1/MENU_ITEMS/food?${params}`);
    ...
    tests.push(await this.runTest('Get all menu items', async () => {
      const response = await fetch(`${BASE_URL}/functions/v1/MENU_ITEMS/items`);
  ...
  private createSummary(functionName: string, tests: TestResult[]): EdgeFunctionTestSuite {
    const summary = {
      total: tests.length,
