...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🚀 NEW Send message request body:', body, 'at', new Date().toISOString());
    ...
    console.log('Parsed values:', { conversationId, receiverId, recipientId, content, messageType });
    ...
    if (!finalRecipientId || !content) {
      console.log('Missing required fields:', { conversationId: !!conversationId, finalRecipientId: !!finalRecipientId, content: !!content });
      return NextResponse.json(
        { error: 'Either conversationId or recipient ID and content are required' },
        { status: 400 }
      );
    }
