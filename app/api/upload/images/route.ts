...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate file type - support both images and content_posts
    const allowedTypes = [
      "image/jpeg",
      "image/jpg",
      "image/png",
    ...
    if (!allowedTypes.includes(file.type)) {
      return NextResponse.json(
        {
          error:
            "Invalid file type. Only JPEG, PNG, WebP images and WebM, MP4 content_posts are allowed.",
        },
        { status: 400 },
      );
    }

    // Validate file size (max 50MB for content_posts, 5MB for images)
    const isVideo = file.type.startsWith("video/");
    const maxSize = isVideo ? 50 * 1024 * 1024 : 5 * 1024 * 1024; // 50MB for content_posts, 5MB for images
