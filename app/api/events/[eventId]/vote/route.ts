...
import {
  WOLFPACK_TABLES,
  WolfpackBackendService,
  WolfpackErrorHandler,
} from "@/lib/services/wolfpack";
...
type ChatMessageInsert =
  Database["public"]["Tables"]["wolfpack_chat_messages"]["Insert"];
  ...
  if (typeof options === "string") {
    try {
      ...
      return Array.isArray(parsed)
        ? parsed.filter((opt) => typeof opt === "string")
        : [];
    } catch {
      return [];
    }
...
async function validateVote(
  ...
  switch (event.event_type) {
    ...
    case "best_costume":
      if (
        voteData.vote_value === undefined || voteData.vote_value < 1 ||
        voteData.vote_value > 10
      ) {
...
async function createVote(
  ...
  if (error || !data) {
    throw new Error(
      `Failed to create vote: ${error?.message || "Unknown error"}`,
    );
  }
...
async function getVoteCounts(
  supabase: Awaited<ReturnType<typeof createClient>>,
  event: EventData,
): Promise<VoteCounts | ContestVoteCounts> {
  ...
  if (
    ...
    const averageRating = totalVotes > 0
      ? validVotes.reduce((sum, vote) => sum + (vote.vote_value || 0), 0) /
        totalVotes
      : 0;
...
async function sendConfirmationMessage(
  ...
  try {
    ...
    switch (event.event_type) {
      ...
      case "best_costume":
        confirmationMessage =
          `${displayName} rated: ${event.title} (${voteData.vote_value}/10)`;
        break;
      case "song_request":
...
export async function GET(
  ...
  try {
    ...
    const userVote = votes.find((vote: VoteQueryResult) =>
    ...
    if (
      ...
      const validVotes = votes.filter((
        vote: VoteQueryResult,
      ): vote is VoteQueryResult & { vote_value: number } =>
        vote.vote_value !== null
      );
      const totalVotes = validVotes.length;
      const averageRating = totalVotes > 0
        ? validVotes.reduce(
          (sum: number, vote: VoteQueryResult & { vote_value: number }) =>
            sum + vote.vote_value,
          0,
        ) / totalVotes
        : 0;
      ...
      votes.forEach((vote: VoteQueryResult) => {
        if (vote.voted_for_id) {
          pollCounts[vote.voted_for_id] = (pollCounts[vote.voted_for_id] || 0) +
            1;
        }
      });
