...
interface TikTokStyleFeedProps {
  ...
  onCreatePost?: () => void;
  onLoadMore?: () => Promise<SocialVideoItem[]>;
  hasMore?: boolean;
  isLoading?: boolean;
...
export default function TikTokStyleFeed({
  ...
  const handleFollowClick = async (userId: string) => {
    ...
    const result = await SocialService.toggleFollow(userId);
  ...
  const handleCommentCountChange = React.useCallback((postId: string, newCount: number) => {
    setVideoStats(prev => {
      const newMap = new Map(prev);
      const currentStats = newMap.get(postId) || { 
        likes_count: 0, 
        comments_count: 0, 
        user_liked: false 
      };
      newMap.set(postId, { ...currentStats, comments_count: newCount });
      return newMap;
    });
  }, []);
  ...
  return (
    <div className="fixed inset-0 bg-black overflow-hidden">
            ...
            <div className="absolute inset-x-0 bottom-0 pb-20 px-4">
              <div className="flex items-center gap-3 mb-3">
                <div className="flex items-center gap-2">
                  ...
                  {video.is_verified && (
                    <span className="bg-blue-600/20 text-blue-400 px-2 py-0.5 rounded-full text-xs font-medium flex items-center gap-1">
      ...
      {showFriendSearch && (
        <FindFriends onCloseAction={() => setShowFriendSearch(false)} />
      )}
    </div>
  );
}