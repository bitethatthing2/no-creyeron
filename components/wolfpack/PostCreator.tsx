...
export function PostCreator({ isOpen, onClose, onSuccess }: PostCreatorProps) {
  ...
  const { hasAccess: canUploadcontent_posts, loading: featureLoading, error: featureError } = 
  ...
  React.useEffect(() => {
    ...
    } else if (!canUploadcontent_posts && !featureLoading) {
  ...
  }, [canUploadcontent_posts, featureLoading, featureError, camera.errorMessage]);
