...
import { Home, ShoppingBag, Calendar, LogIn, Shield, Info, Settings } from 'lucide-react';
...
// import { usecontent_comments } from '@/lib/contexts/content_commentsContext';
...
export const BottomNav = () => {
  ...
  // Simplified - no DJ functionality
  const { isAdmin } = useAdminAccess();
  const { user } = useAuth();
  const [isMounted, setIsMounted] = React.useState(false);
  // const { iscontent_commentsOpen } = usecontent_comments();
  // const iscontent_commentsOpen = false; // Temporarily disabled until context is working
  ...
  }, [isMounted, user, canCheckout, isAdmin]);
  ...
  return (
    <>
      ...
      <nav className={`fixed bottom-0 left-0 right-0 h-16 border-t bg-background/80 backdrop-blur-md ${getZIndexClass('BOTTOM_NAV')} safe-area-inset-bottom`}>
        ...
        {canCheckout && (
            <div
              className={`h-1 w-12 rounded-full bg-gradient-to-r from-primary to-primary-light shadow-lg animate-pulse ${styles.wolfpackStatusIndicator}`}
            />
        )}
      </nav>
    </>
